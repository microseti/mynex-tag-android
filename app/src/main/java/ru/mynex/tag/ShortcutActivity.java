...
public class ShortcutActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!executeAction(getIntent())) {
            ...
            listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int data, long id) {
                    switch (data) {
                        case 0:
                            setShortcutResult(items[data], R.mipmap.ic_start, ACTION_START);
                            break;
                        case 1:
                            setShortcutResult(items[data], R.mipmap.ic_stop, ACTION_STOP);
                            break;
                        case 2:
                            setShortcutResult(items[data], R.mipmap.ic_sos, ACTION_SOS);
                            break;
                    }
                    finish();
                }
    ...
    private void sendAlarm() {
        ...
        // TrackingController tc = TrackingService.class.getTrackingController();

        // PositionProviderFactory.create(this, new PositionProvider.PositionListener() {
        //     @Override
        //     public void onPositionUpdate(Data data) {
        //         String pack = ProtocolFormatter.formatPackage(data, ALARM_SOS);

        //         RequestManager.sendRequestAsync(pack, new RequestManager.RequestHandler() {
        //             @Override
